#!/usr/bin/env python3
"""
Train a hybrid physics-informed world model (WorldModelPhysicsHybrid) that predicts next observation from (observation, action).
...
- Exposes physics-specific hyperparameters like dt and residual MLP
"""
...
try:
    from MLP.world_model_with_physics import WorldModelPhysicsHybrid
except Exception:
    from world_model_with_physics import WorldModelPhysicsHybrid
...
def train_world_model_physics(
    ...
    for tol in tolerances:
        ...
        print(f"Tolerance {tol}: Train acc={train_acc:.4f} | " f"Val acc={val_acc:.4f}")
...
Examples:
  ...
  python MLP/train_world_model_physics.py --buffer_path buffer.pt --dt 0.02 --no_residual --cfrc_weight 0.05
    ...
    parser.add_argument(
        "--no_residual",
        action="store_true",
        help="Disable residual MLP on top of physics basis",
    )
    parser.add_argument(
        ...
        nargs="+",
        default=[64, 64],
        help="Residual MLP hidden layer dimensions (adjust for parameter budget)",
    )
    parser.add_argument(
        "--activation",
        type=str,
        ...
        help="Activation for residual/cfrc heads",
    )
    parser.add_argument(
        "--cfrc_weight",
        type=float,
    ...
    # The hybrid model is designed for InvertedDoublePendulum-v4: obs_dim=11, action_dim=1
    if obs_dim != 11 or action_dim != 1:
        raise ValueError(
            f"WorldModelPhysicsHybrid expects obs_dim=11 and action_dim=1, got obs_dim={obs_dim}, action_dim={action_dim}"
        )
    ...
    model = WorldModelPhysicsHybrid(
        obs_dim=obs_dim,
        action_dim=action_dim,
        dt=float(args.dt),
        use_residual=(not args.no_residual),
        residual_hidden_dims=args.residual_hidden_dims,
